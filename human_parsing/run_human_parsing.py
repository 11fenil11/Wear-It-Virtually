...
if __name__ == '__main__':
    ...
    print("START JPPNet Human Parsing")
    # Run JPPNet for human parsing
    os.system('cd LIP_JPPNet;python evaluate_parsing_JPPNet-s2.py; cd ..;')
    print("COmpleted Human Parsing")
    # Convert output to .mat
    img = cv2.imread(r'./LIP_JPPNet/output/parsing/val/{}'.format(os.path.splitext(filename)[0]+'.png'))
    #cv2.imshow("Temp",img)
    seg_dict = {'segment':img[:,:,0]} # Only need one channel (R, G, or B)
    #print(img)
    scipy.io.savemat(r'./output/{}'.format(os.path.splitext(filename)[0]+'.mat'), seg_dict)
